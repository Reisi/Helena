...
/* Private defines -----------------------------------------------------------*/
#define MAINBEAM                0   // first driver of i2c base is used for main beam
#define HIGHBEAM                1   // second driver of i2c base is used for high beam
...
static uint32_t readConverterData(light_status_t* pStatus)
{
    ...
    {
        pStatus->mainBeam.dutycycle = Buffer[0]>>4 ? true : false;
        pStatus->highBeam.dutycycle = Buffer[2]>>4 ? true : false;
        i = ((Buffer[0] & 0x0F)<<8) | Buffer[1];
        pStatus->currentMainBeam = REGISTERVALUE_TO_AMPEREQ10(i);
        i = ((Buffer[2] & 0x0F)<<8) | Buffer[3];
        pStatus->currentHighBeam = REGISTERVALUE_TO_AMPEREQ10(i);
...
static void limiter(light_status_t * pStatus, uint8_t *pMainBeam, uint8_t *pHighBeam)
{
    ...
    if (*pMainBeam > currentLimits[MAINBEAM])
    {
        *pMainBeam = currentLimits[MAINBEAM];
    ...
    if (*pHighBeam > currentLimits[HIGHBEAM])
    {
        *pHighBeam = currentLimits[HIGHBEAM];
...
static void validateCurrentLimit(currentLimit_t* pLimits)
{
    ...
    currentLimits[MAINBEAM] = pLimits->mainBeam;

    if (pLimits->highBeam > designLimit)
        pLimits->highBeam = designLimit;
    currentLimits[HIGHBEAM] = pLimits->highBeam;
}
...
static void readStorage()
{
    ...
    storage.currentLimits.mainBeam = OUTPUT_DEFAULT_LIMT;
    storage.currentLimits.highBeam = OUTPUT_DEFAULT_LIMT;
...
uint32_t light_UpdateTargets(const light_mode_t* pMode, const light_status_t* *ppStatus)
{
    ...
    if (pMode->setup.mainBeam || pMode->setup.highBeam)
        calculateTarget(pMode, &targets[MAINBEAM], &targets[HIGHBEAM]);
...
void light_Execute()
{
    ...
    if (targets[MAINBEAM] == 0 && targets[HIGHBEAM] == 0)          // if light is off, nothing more to do
        return;

    uint8_t limitedTargets[2];

    limitedTargets[MAINBEAM] = targets[MAINBEAM];                 // run targets through limiter
    limitedTargets[HIGHBEAM] = targets[HIGHBEAM];
    limiter(&lightStatus, &limitedTargets[MAINBEAM], &limitedTargets[HIGHBEAM]);
        ...
        if (storage.drvConfig.rev == LIGHT_DRIVERREV11)
        {
            ...
            if(errCode == NRF_SUCCESS)
            {
                if (storage.drvConfig.mainBeamCount)
                    storage.drvConfig.mainBeamCount = buffer[0] < (HELENABASE_DC_MAX * 0.25) ? 0 : buffer[0] < (HELENABASE_DC_MAX * 0.75) ? 1 : 2;
                if (storage.drvConfig.highBeamCount)///^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^- just bugfix
                    storage.drvConfig.highBeamCount = buffer[1] < (HELENABASE_DC_MAX * 0.25) ? 0 : buffer[1] < (HELENABASE_DC_MAX * 0.75) ? 1 : 2;
            }
        }
    }
...
uint32_t light_GetLimits(q8_t* pMainBeamLimit, q8_t* pHighBeamLimit)
{
    ...
    *pMainBeamLimit = storage.currentLimits.mainBeam;
    *pHighBeamLimit = storage.currentLimits.highBeam;
...
{
    storage.currentLimits.mainBeam = mainBeamLimit;
    storage.currentLimits.highBeam = highBeamLimit;
